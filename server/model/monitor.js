...
const { CecilefyUptimeServer } = require("../cecilefy-uptime-server");
...
class Monitor extends BeanModel {
    ...
    toJSON(preloadData = {}, includeSensitiveData = true) {
        ...
        if (this.type === "real-browser") {
            screenshot = "/screenshots/" + jwt.sign(this.id, CecilefyUptimeServer.getInstance().jwtSecret) + ".png";
        }
    ...
    async start(io) {
        ...
        const beat = async () => {
            ...
            // See https://github.com/yardanshaq/cecilefy-uptime/pull/3961#issuecomment-1804149144
            ...
            try {
                if (await Monitor.isUnderMaintenance(this.id)) {
                ...
                } else if (this.type in CecilefyUptimeServer.monitorTypeList) {
                    let startTime = dayjs().valueOf();
                    const monitorType = CecilefyUptimeServer.monitorTypeList[this.type];
                    await monitorType.check(this, bean, CecilefyUptimeServer.getInstance());
                    if (!bean.ping) {
                        bean.ping = dayjs().valueOf() - startTime;
                    }
                    ...
                    bean.msg = await kafkaProducerAsync(
                        ...
                        this.kafkaProducerMessage,
                        {
                            ...
                            clientId: `Cecilefy-Uptime/${version}`,
                            interval: this.interval,
                        },
            ...
            if (isImportant) {
                ...
                await CecilefyUptimeServer.getInstance().sendMaintenanceListByUserID(this.user_id);
        ...
        const safeBeat = async () => {
            try {
                ...
                console.trace(e);
                CecilefyUptimeServer.errorLog(e, false);
                log.error("monitor", "Please report to https://github.com/yardanshaq/cecilefy-uptime/issues");
        ...
        if (!isFirstBeat || bean.status === DOWN) {
            ...
            for (let notification of notificationList) {
                try {
                    ...
                    heartbeatJSON["timezone"] = await CecilefyUptimeServer.getInstance().getTimezone();
                    heartbeatJSON["timezoneOffset"] = CecilefyUptimeServer.getInstance().getTimezoneOffset();
    ...
    static async isUnderMaintenance(monitorID) {
        ...
        for (const maintenanceID of maintenanceIDList) {
            const maintenance = await CecilefyUptimeServer.getInstance().getMaintenance(maintenanceID);
