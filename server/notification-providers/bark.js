...
const barkNotificationAvatar = "https://github.com/yardanshaq/cecilefy-uptime/raw/master/public/icon.png";
...
class Bark extends NotificationProvider {
    ...
    async send(notification, msg, monitorJSON = null, heartbeatJSON = null) {
        ...
        if (msg != null && heartbeatJSON != null && heartbeatJSON["status"] === UP) {
            let title = "CecilefyUptime Monitor Up";
            return await this.postNotification(notification, title, msg, barkEndpoint);
        }

        if (msg != null && heartbeatJSON != null && heartbeatJSON["status"] === DOWN) {
            let title = "CecilefyUptime Monitor Down";
            return await this.postNotification(notification, title, msg, barkEndpoint);
        }

        if (msg != null) {
            let title = "CecilefyUptime Message";
            return await this.postNotification(notification, title, msg, barkEndpoint);
        }
    }
    ...
    additionalParameters(notification) {
        // set icon to cecilefy uptime icon, 11kb should be fine
    ...
    async postNotification(notification, title, subtitle, endpoint) {
        ...
        if (notification.apiVersion === "v1" || notification.apiVersion == null) {
            ...
            result = await axios.post(`${endpoint}/push`, {
                ...
                group: notification.barkGroup || "CecilefyUptime", // default group is CecilefyUptime
            }, config);
        }
